...
parser.add_argument("--a", type=float, default=30, help="weight of topology loss")
...
parser.add_argument("--save_dir", type=str, default='best_model_no_top', help="directory where you save models")
...
def sample_image(n_row, batches_done, current_epoch):
    ...
    gen_imgs = decoder(z)
    save_image(gen_imgs.data, "images_no_top/epoch_%d.png" % current_epoch, nrow=n_row, normalize=True)
...
for epoch in range(opt.n_epochs):
    ...
    for j, (valid_imgs, _) in enumerate(validloader):
        ...
        valid_g_loss = 0.001 * adversarial_loss(discriminator(encoded_imgs), valid) + 0.999 * pixelwise_loss(
            decoded_imgs, real_imgs
        )
