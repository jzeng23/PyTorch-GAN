...
def sample_image(n_row, batches_done, current_epoch, img):
    ...
    gen_imgs = decoder(z)
    save_image(img.data, "images/images_retina/epoch_%d.png" % current_epoch, nrow=n_row, normalize=True)
...
for epoch in range(opt.n_epochs):
    ...
    for i, (imgs, neighborhoods, cores, diffs, train_beta0_goal, train_beta1_goal) in enumerate(trainloader):
        ...
        g_loss = diff_loss + ngh_loss + core_loss + top_loss + 0.001 * adversarial_loss(discriminator(encoded_imgs), valid)
    ...
    for j, (valid_imgs, valid_neighborhoods, valid_cores, valid_diffs, valid_beta0_goal, valid_beta1_goal) in enumerate(validloader):
        ...
        # Loss measures generator's ability to fool the discriminator
        valid_g_loss = 40 * diff_loss + 2*ngh_loss + core_loss + top_loss + 0.001 * adversarial_loss(discriminator(encoded_imgs), valid)
    ...
    writer.add_scalars('LR 0.0002', {
            ...
            'Train Diff': epoch_diff_loss / len(trainloader),
            'Train Topology': epoch_top_loss / len(trainloader),
