...
parser.add_argument("--save_dir", type=str, default='latest_model/loss_abs_value/lr_0.0002', help="directory where you save models")
...
def sample_image(n_row, batches_done, current_epoch, img):
    ...
    # Sample noise
    save_image(img.data, "images/loss_abs_value/lr_0.0002/epoch_%d.png" % current_epoch, nrow=n_row, normalize=True)
...
for epoch in range(opt.n_epochs):
    ...
    for i, (imgs, neighborhoods, cores, train_beta0_goal, train_beta1_goal, train_nc_size, train_core_size) in enumerate(trainloader):
        ...
        if batches_done % opt.sample_interval == 0:
            sample_image(n_row=opt.batch_size, batches_done=batches_done, current_epoch=epoch, img=decoded_imgs.clone().detach())
    ...
    if epoch_valid_g_loss < best_val_loss:
        ...
        #torch.save(discriminator, os.path.join(opt.save_dir, 'discriminator_%03d.pth' % (epoch)))
    '''      
    writer.add_scalars('LR=0.0002, Abs Val 0.005, 0.001, epsilon=15', {
