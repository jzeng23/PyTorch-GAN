...
parser.add_argument("--save_dir", type=str, default='best_model_no_compression', help="directory where you save models")
opt = parser.parse_args()
print(opt)

img_shape = (opt.channels, opt.img_size, opt.img_size)
img_shape_flattened = int(np.prod(img_shape))
...
def reparameterization(mu, logvar):
    ...
    sampled_z = Variable(Tensor(np.random.normal(0, 1, (mu.size(0), img_shape_flattened))))
    z = sampled_z * std + mu
    return z
...
class Encoder(nn.Module):
    def __init__(self):
        ...
        self.model = nn.Sequential(
            nn.Linear(img_shape_flattened, img_shape_flattened),
            nn.BatchNorm1d(img_shape_flattened),
            nn.LeakyReLU(0.2, inplace=True),
        )

        self.mu = nn.Linear(img_shape_flattened, img_shape_flattened)
        self.logvar = nn.Linear(img_shape_flattened, img_shape_flattened)
...
class Discriminator(nn.Module):
    def __init__(self):
        ...
        self.model = nn.Sequential(
            nn.Linear(img_shape_flattened, 512),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Linear(512, 256),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Linear(256, 1),
...
def sample_image(n_row, batches_done, current_epoch):
    ...
    # Sample noise
    z = Variable(Tensor(np.random.normal(0, 1, (n_row ** 2, img_shape_flattened))))
    gen_imgs = decoder(z)
    save_image(gen_imgs.data, "imagest_no_compression/epoch_%d.png" % current_epoch, nrow=n_row, normalize=True)
...
for epoch in range(opt.n_epochs):
    ...
    for i, (imgs, _) in enumerate(trainloader):
        ...
        top_loss = top_loss / (3*decoded_imgs.size(0))
        ...
        z = Variable(Tensor(np.random.normal(0, 1, (imgs.shape[0], img_shape_flattened))))
