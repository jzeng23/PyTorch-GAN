...
epsilon = 15
epoch = 6100
barcodes_dir = 'implementations/aae/barcode/loss_mse/ExponentialLR/0.00017/gamma_0.9996/alpha/diff_thresholds_mini_dataset'
n = 10
...
for i in range(n):
    ...
    for k in range(barcode1.shape[0]):
        ...
        if pair[0] >= C and pair[1] < N:
            beta1 += 1
    betas[i, 1] = beta1
np.savetxt('data/output_betas_epoch_%d.csv' % epoch, betas, fmt='%1.0f', delimiter=',')
